...
static const char *USER_KEYS[MAX_USERS] = {
...
esp_err_t users_init() {
    ...
    if (err != ESP_OK)
        printf("error opening users nvs: %s\n", esp_err_to_name(err));

    // get device id from memory
    err = nvs_get_str(users_handle, DEVICE_ID_KEY, device_id,
    ...
    for (int i = 1; i < users_array.length; i++) {
        ...
        nvs_get_str(users_handle, user_string, users_array.users[i].id,
                    strlen(user_string) + 1);
    }
...
void singleton_add_user(char *new_user_id) {
    ...
    err = nvs_set_str(users_handle, USER_KEYS[index],
                      users_array.users[index].id);
    if (err != ESP_OK) {
        printf("Error adding user id to nvs: %s\n", esp_err_to_name(err));
    }

    err = nvs_commit(users_handle);
    if (err != ESP_OK) {
        printf("Error commiting user id to nvs: %s\n", esp_err_to_name(err));
    }
